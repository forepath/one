import { Inject, Injectable, Logger } from '@nestjs/common';

import type { BillingProvisioningProvider } from '@forepath/decabill/backend/util-plugin-host';
import { BILLING_PROVISIONING_PROVIDER_REGISTRY } from '@forepath/decabill/backend/util-plugin-host';
import { ProviderRegistry } from '@forepath/shared/backend/util-extension-core';

import { ServerInfo } from '@forepath/decabill/backend/util-plugin-host';

const DIGITALOCEAN_PUBLIC_IP_POLL_INTERVAL_MS = 2000;
const DIGITALOCEAN_PUBLIC_IP_MAX_ATTEMPTS = 30;

@Injectable()
export class ProvisioningService {
  private readonly logger = new Logger(ProvisioningService.name);

  constructor(
    @Inject(BILLING_PROVISIONING_PROVIDER_REGISTRY)
    private readonly provisioningRegistry: ProviderRegistry<BillingProvisioningProvider>,
  ) {}

  private delay(ms: number): Promise<void> {
    return new Promise((resolve) => setTimeout(resolve, ms));
  }

  private resolveProvider(provider: string): BillingProvisioningProvider | null {
    if (!this.provisioningRegistry.hasProvider(provider)) {
      return null;
    }

    return this.provisioningRegistry.getProvider(provider);
  }

  /**
   * Resolves a public IPv4 for Cloudflare DNS after provision.
   * DigitalOcean often omits public v4 on the first droplet GET; poll until it appears or timeout.
   */
  async ensurePublicIpForDns(
    provider: string,
    serverId: string,
    initial: ServerInfo | null | undefined,
  ): Promise<string | undefined> {
    let info = initial ?? (await this.getServerInfo(provider, serverId));

    if (info?.publicIp) {
      return info.publicIp;
    }

    if (provider !== 'digital-ocean') {
      return undefined;
    }

    for (let attempt = 1; attempt < DIGITALOCEAN_PUBLIC_IP_MAX_ATTEMPTS; attempt++) {
      await this.delay(DIGITALOCEAN_PUBLIC_IP_POLL_INTERVAL_MS);
      info = await this.getServerInfo(provider, serverId);

      if (info?.publicIp) {
        return info.publicIp;
      }
    }

    this.logger.warn(
      `Timed out waiting for public IPv4 on DigitalOcean droplet ${serverId} after approximately ${
        DIGITALOCEAN_PUBLIC_IP_MAX_ATTEMPTS * DIGITALOCEAN_PUBLIC_IP_POLL_INTERVAL_MS
      }ms`,
    );

    return undefined;
  }

  async provision(provider: string, config: { [key: string]: unknown }): Promise<{ serverId: string } | null> {
    const resolved = this.resolveProvider(provider);

    if (!resolved) {
      return null;
    }

    const result = await resolved.provision(config);

    if (!result || typeof result !== 'object' || !('serverId' in result)) {
      return null;
    }

    return result as { serverId: string };
  }

  async deprovision(provider: string, serverId: string): Promise<void> {
    const resolved = this.resolveProvider(provider);

    if (!resolved) {
      return;
    }

    await resolved.deprovision(serverId);
  }

  async getServerInfo(provider: string, serverId: string): Promise<ServerInfo | null> {
    const resolved = this.resolveProvider(provider);

    if (!resolved) {
      return null;
    }

    return await resolved.getServerInfo(serverId);
  }

  async startServer(provider: string, serverId: string): Promise<void> {
    const resolved = this.resolveProvider(provider);

    if (!resolved) {
      return;
    }

    await resolved.startServer(serverId);
  }

  async stopServer(provider: string, serverId: string): Promise<void> {
    const resolved = this.resolveProvider(provider);

    if (!resolved) {
      return;
    }

    await resolved.stopServer(serverId);
  }

  async restartServer(provider: string, serverId: string): Promise<void> {
    const resolved = this.resolveProvider(provider);

    if (!resolved) {
      return;
    }

    await resolved.restartServer(serverId);
  }
}
