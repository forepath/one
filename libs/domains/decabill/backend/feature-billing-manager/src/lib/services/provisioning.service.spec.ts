...
describe('ProvisioningService', () => {
  const digitalocean: jest.Mocked<
    Pick<
      BillingProvisioningProvider,
      'provision' | 'deprovision' | 'getServerInfo' | 'startServer' | 'stopServer' | 'restartServer'
    >
  > = {
  ...
  function createService(
    hetzner: Partial<BillingProvisioningProvider>,
    digitalOcean: Partial<BillingProvisioningProvider> = digitalocean,
  ): ProvisioningService {
    const registry = new ProviderRegistry<BillingProvisioningProvider>();

    registry.register('hetzner', hetzner as BillingProvisioningProvider);
    registry.register('digital-ocean', digitalOcean as BillingProvisioningProvider);
  ...
  it('routes to Hetzner provisioning', async () => {
    ...
    const service = createService(hetzner);
  ...
  it('routes to DigitalOcean provisioning', async () => {
    ...
    expect(digitalocean.provision).toHaveBeenCalledWith({ name: 'test' });
  });
  ...
  it('routes deprovision to Hetzner', async () => {
    ...
    expect(hetzner.deprovision).toHaveBeenCalledWith('server-123');
  });
  ...
  it('routes deprovision to DigitalOcean', async () => {
    ...
    expect(digitalocean.deprovision).toHaveBeenCalledWith('server-456');
  });
  ...
  it('ignores deprovision for unknown provider', async () => {
    ...
    expect(hetzner.deprovision).not.toHaveBeenCalled();
    expect(digitalocean.deprovision).not.toHaveBeenCalled();
  });

  it('returns null for unknown provider', async () => {
    const hetzner = { provision: jest.fn() };
    const service = createService(hetzner);
  ...
  it('routes startServer to DigitalOcean', async () => {
    const hetzner = { startServer: jest.fn() };
    const service = createService(hetzner);
  ...
  it('routes stopServer to DigitalOcean', async () => {
    const hetzner = { stopServer: jest.fn() };
    const service = createService(hetzner);
  ...
  it('routes restartServer to DigitalOcean', async () => {
    const hetzner = { restartServer: jest.fn() };
    const service = createService(hetzner);
  ...
  it('ensurePublicIpForDns polls DigitalOcean until public IPv4 appears', async () => {
    ...
    try {
      ...
      const service = createService(hetzner, digitalOcean);
