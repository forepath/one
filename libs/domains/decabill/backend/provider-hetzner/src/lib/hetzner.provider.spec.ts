...
describe('HetznerProvider', () => {
  ...
  it('provisions a server', async () => {
    ...
    const service = new HetznerProvider();
  ...
  it('deprovisions a server', async () => {
    ...
    await service.deprovision('12345');
  ...
  it('throws on deprovision error', async () => {
    ...
    await expect(service.deprovision('12345')).rejects.toThrow(BadRequestException);
  });
  ...
  describe('getServerTypes', () => {
    it('fetches and maps Hetzner server types when token is set', async () => {
    ...
    it('filters out deprecated server types', async () => {
      mockedAxios.get.mockResolvedValue({
        data: {
          server_types: [
    ...
    it('throws when HETZNER_API_TOKEN is not set', async () => {
      delete process.env.HETZNER_API_TOKEN;
      const service = new HetznerProvider();

      await expect(service.getServerTypes()).rejects.toThrow(BadRequestException);
      await expect(service.getServerTypes()).rejects.toThrow('HETZNER_API_TOKEN');
    });
  });
});
