...
import { DockerService } from '@forepath/agenstra/backend/feature-agent-manager/plugin-deps';
import {
  AgentProvider,
...
export class CursorProvider implements AgentProvider {
  ...
  getModelsListCommand(): string {
    return CursorProvider.LIST_MODELS_COMMAND;
  }
  ...
  private static stripAnsiSequences(text: string): string {
    return text.replace(CursorProvider.ANSI_CSI_ESCAPE, '');
  }
  ...
  toModelsList(result: string): AgentProviderModels {
    ...
    const cleaned = CursorProvider.stripAnsiSequences(result);
    const sep = CursorProvider.MODEL_LINE_SEPARATOR;
