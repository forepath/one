...
export class HetznerProvisioningProvider implements ProvisioningProvider {
  private readonly logger = new Logger(HetznerProvisioningProvider.name);
   ...
   */
  getType(): string {
    return HetznerProvisioningProvider.TYPE;
  }
  ...
  async getServerTypes(): Promise<ServerType[]> {
    ...
    try {
      const response = await axios.get<{ server_types: HetznerServerType[] }>(
        `${HetznerProvisioningProvider.API_BASE_URL}/server_types`,
        {
          headers: {
  ...
  async provisionServer(options: ProvisionServerOptions): Promise<ProvisionedServer> {
    ...
    try {
      ...
      const createResponse = await axios.post<{ server: HetznerServer; action: HetznerAction }>(
        `${HetznerProvisioningProvider.API_BASE_URL}/servers`,
        {
          name: options.name,
  ...
  async deleteServer(serverId: string): Promise<void> {
    ...
    try {
      await axios.delete(`${HetznerProvisioningProvider.API_BASE_URL}/servers/${serverId}`, {
        headers: {
          Authorization: `Bearer ${this.apiToken}`,
        },
      });
  ...
  async getServerInfo(serverId: string): Promise<ServerInfo> {
    ...
    try {
      const response = await axios.get<{ server: HetznerServer }>(
        `${HetznerProvisioningProvider.API_BASE_URL}/servers/${serverId}`,
        {
          headers: {
            Authorization: `Bearer ${this.apiToken}`,
          },
