...
describe('ChatFilterDbRegexIncomingProvider', () => {
  let filter: ChatFilterDbRegexIncomingProvider;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ChatFilterDbRegexIncomingProvider,
        { provide: RegexFilterRulesEvaluateService, useValue: evaluateService },
      ],
    }).compile();

    filter = module.get(ChatFilterDbRegexIncomingProvider);
  });
