...
import type { ExternalImportConfigEntity } from '@forepath/agenstra/backend/feature-agent-controller';
import {
...
import {
  ...
  isJiraAdfDocument,
  jiraAdfToMarkdown,
} from './utils/atlassian-format-to-markdown';
import {
  atlassianFetchJson,
...
} from './utils/atlassian-rest.util';
import { atlassianWikiMarkupToMarkdown, looksLikeAtlassianWikiMarkup } from './utils/atlassian-wiki-markup-to-markdown';
import { buildConfluenceImportSearchCql } from './utils/confluence-import-cql.util';
import {
  confluenceTrimmedImportChain,
  extractConfluenceInternalPageIdsFromHtml,
} from './utils/confluence-import-structure.util';
import { confluenceStorageHtmlToMarkdown } from './utils/confluence-storage-html-to-markdown';
import { buildJiraIssueSearchJql, resolveJiraBoardIdForAgileApi } from './utils/jira-import-jql.util';
...
export class AtlassianImportProvider implements ExternalContextImportProvider {
  ...
  async runImport(ctx: ExternalContextImportRunContext): Promise<ExternalContextImportRunResult> {
    ...
    const config = ctx.config as ExternalImportConfigEntity;
