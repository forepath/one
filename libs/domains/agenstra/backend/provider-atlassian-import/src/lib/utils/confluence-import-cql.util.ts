/**
 * Builds the CQL sent to Confluence content/search, combining the user query with
 * optional space key and root page scope from import config.
 */
...
/** Double-quoted CQL string literal with minimal escaping. */
function cqlQuotedString(value: string): string {
  return `"${value.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`;
}

/** Confluence CQL uses bare numeric ids in examples; quote non-numeric ids defensively. */
