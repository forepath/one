 ...
 */
@Injectable()
export class DigitalOceanProvisioningProvider implements ProvisioningProvider {
  private readonly logger = new Logger(DigitalOceanProvisioningProvider.name);
   ...
   */
  getType(): string {
    return DigitalOceanProvisioningProvider.TYPE;
  }
  ...
  async getServerTypes(): Promise<ServerType[]> {
    ...
    try {
      const response = await axios.get<{ sizes: DigitalOceanSize[] }>(
        `${DigitalOceanProvisioningProvider.API_BASE_URL}/sizes`,
        {
          headers: {
  ...
  async provisionServer(options: ProvisionServerOptions): Promise<ProvisionedServer> {
    ...
    try {
      ...
      const createResponse = await axios.post<{ droplet: DigitalOceanDroplet }>(
        `${DigitalOceanProvisioningProvider.API_BASE_URL}/droplets`,
        {
          name: options.name,
  ...
  async deleteServer(serverId: string): Promise<void> {
    ...
    try {
      await axios.delete(`${DigitalOceanProvisioningProvider.API_BASE_URL}/droplets/${serverId}`, {
        headers: {
          Authorization: `Bearer ${this.apiToken}`,
        },
      });
  ...
  async getServerInfo(serverId: string): Promise<ServerInfo> {
    ...
    try {
      const response = await axios.get<{ droplet: DigitalOceanDroplet }>(
        `${DigitalOceanProvisioningProvider.API_BASE_URL}/droplets/${serverId}`,
        {
          headers: {
            Authorization: `Bearer ${this.apiToken}`,
          },
  ...
  private async getSshKeyIds(sshKey: string): Promise<number[]> {
    try {
      ...
      const response = await axios.get<{ ssh_keys: DigitalOceanSshKey[] }>(
        `${DigitalOceanProvisioningProvider.API_BASE_URL}/account/keys`,
        {
          headers: {
            Authorization: `Bearer ${this.apiToken}`,
          },
