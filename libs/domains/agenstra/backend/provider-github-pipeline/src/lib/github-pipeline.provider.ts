 ...
 */
@Injectable()
export class GithubPipelineProvider implements PipelineProvider {
  private readonly logger = new Logger(GithubPipelineProvider.name);
   ...
   */
  getType(): string {
    return GithubPipelineProvider.TYPE;
  }
  ...
  private getApiBaseUrl(credentials: PipelineProviderCredentials): string {
    return credentials.baseUrl || GithubPipelineProvider.API_BASE_URL;
  }
